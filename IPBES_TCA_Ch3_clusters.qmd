...
The key papers should describe only one cluster, and not multiple clusters. In other words,
a key paper should be in only one cluster. If a key-paper is in multiple clusters, this would lead to an overlap of the snowball corpus which is not caused by the cluster itself, but by the key paper which is in multiple clusters.
...
To download the high resolution graph, please [click here](figures/snowball_cited_by_count.pdf).
...
- when moving your mouse over a node, the title author and year of the paper is shown.
...
htmlwidgets::saveWidget(p, file = file.path("figures", "common_l3.html"), selfcontained = TRUE)
...
htmlwidgets::saveWidget(p, file = file.path("figures", "common_l3_equal.html"), selfcontained = TRUE)
...
Here I will show the overlap between the combined cluster corpi and the General corpus based on the six key papers.
