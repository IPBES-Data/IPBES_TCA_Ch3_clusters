---
title: 'Chapter 3 - Aproaches and General snowball search'
...
author:
  ...
  - name: 
        ...
        given:   Catherine
    id: cs
    orcid: 0000-0001-5136-1517
    roles: [author]
  - name: 
        family:  Mansur
        given:   Andressa
    id: am
    orcid: 0000-0003-4104-3085
    roles: [author]
license: "CC BY"
...
- [Hakon Stokland](mailto:hakon.stokland@nina.no>) 
...
Setup the environment for the compilation of the report.
...
## Schematic Overview of the TCA Chapter 3 Approaches (cluster) Snowball SearchesSearches and Analysis
...
The `data` folder is not included in this deposit, but in a separate deposit at [10.5281/zenodo.11386248](https://doi.org/10.5281/zenodo.11386248).
...
if (!dir.exists("data")) {
  ...
  url <- paste0("https://zenodo.org/record/", "11386248", "/files/data.zip")
...
In this section the ddata is gathered and analysed. Results are shown in the Results section below.

## Verification of key papers

We have the following number of keypapers per cluster / approach:

```{r}
sapply(cluster, function(x) {
  length(x$dois)
})
```
...
These papers will be excluded from the analysis due to be present in more tha=en one cluster / approach..
...
This results in the folowing number of key papers used for the dnowball search:
...
The snowball searc h resulted in the folowing number of papers in each corpus for the cluster / approaches
```{r}
sapply(cluster[1:6], function(x) {
  nrow(x$snowball_1$nodes)
})
```
